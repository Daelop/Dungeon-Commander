...
export function handleDamage(context: {
  character: Character;
  baseDamage: number;
  type: DamageType;
  scaling: scaling;
  ...
  targets.forEach((target) => {
    const damageRoll = Math.floor(
      (Math.random() * (1.2 - 0.8) + 0.8) * scaledDamage
    );
    target.takeDamage(damageRoll, type);
  });
}

export function handleHeal(context: {
  character: Character;
  baseHeal: number;
  scaling: scaling;
  targets: Character[];
  ...
  targets.forEach((target) => {
    ...
    const healRoll = Math.floor(
      (Math.random() * (1.2 - 0.8) + 0.8) * healedAmount
    );
...
export function snapshotHoT(context: {
  character: Character;
  baseHeal: number;
  scaling?: scaling;
}): number {
...
export function snapshotDoT(context: {
  character: Character;
  baseDamage: number;
  type: DamageType;
  scaling?: scaling;
...
export function handleHotTick(context: {
  healedAmount: number;
  target: Character;
}): void {
...
export function handleDotTick(context: {
  damage: number;
  type: DamageType;
  target: Character;
}): void {
...
export function handleStatusEffect(
  statusEffectInput: statusEffectInput,
  target: Character,
